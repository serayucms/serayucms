...
//        $cs->registerScriptFile('http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.21/jquery-ui.min.js');
    ...
    public function run()
    {
        ...
        $inputOptions = array('id' => $id, 'style' => 'float:left;', "placeholder"=>$this->model->generateAttributeLabel($this->attribute) , 'append' => TbHtml::link('Browse', "#" , array('id' => $id . 'browse')) /*, 'readonly' => 'readonly'*/);
        if ($this->hasModel())
            echo TbHtml::activeTextField($this->model, $this->attribute, $inputOptions);
        else
            echo TbHtml::textField($name, $this->value, $inputOptions);
